...
$tpl = new TemplatePower("skin/info_leader.tpl");
$tpl->prepare();
$id = intval($_GET['id']);
$idc = intval($_GET['idc']);
$pid = intval($_GET['pid']);
$module = new InfoLeader();
$module->_int();
$tpl->printToScreen();

class InfoLeader extends cat_tree {

    private $page = "Trang nội dung";
    private $item = "Lãnh đạo";
    private $table = "info_leader";
    private $id_item = "id_info";
    private $par_page = "info_leader";
    private $numberPageShow = 8;
    private $numberItemPage = 30;
    private $thumbwidth = 80;
    private $data_type = 'info_leader';
    ...
    private function save() {
        ...
        if ($data) {
            ...
            if (intval($lastid) > 0) {
                ...
                Message::showMessage("success", "Đã thực hiện ! ($lastid)");
            }
        }
    }

    private function update($id) {
        global $DBi, $search_obj;
        $id = intval($id);
        $data = $this->getData();
        if ($data) {
    ...
    function getData($update = 0, $id = 0) {
        ...
        $data['dob'] = compile_post('dob');
        $data['branch'] = compile_post('branch');
        $data['position'] = compile_post('position');

        $data['active'] = intval(compile_post('active'));
        $data['level'] = intval(compile_post('level'));
