...
$module = new InfoLichSuHinhThanh;
$module->_int();
$tpl->printToScreen();

class InfoLichSuHinhThanh extends cat_tree {
    private $page = "Trang Lịch sử hình thành";
    private $item = "Lịch sử";
    private $table = "info_lichsu";
    private $id_item = "id_info";
    private $par_page = "info_lichsu";
    private $numberPageShow = 8;
    private $numberItemPage = 30;
    private $thumbwidth = 80;
    private $data_type = 'info_lichsu';
    ...
    private function showUpdate() {
        ...
        if ($rs = $DBi->fetch_array($db)) {
            ...
            $tpl->assign($rs['template_name'], "selected" );
    ...
    private function save() {
        ...
        if ($data) {
            ...
            if (intval($lastid) > 0) {
                ...
                Message::showMessage("success", "Đã cập nhật ! ($lastid)");
            }
        }
    }
    ...
    function insert_multi_image($id_info) {
        global $DBi, $objSysImage;
        $objSysImage->deleteMulti('', 'info', 'id_info', $id_info, "info");
    ...
    function show_multi_image($id_info) {
        global $DBi, $tpl, $dir_path, $cache_image_path, $objSysImage;
        $db = $objSysImage->getList('', "info", "id_info", $id_info, "info");
        foreach ($db as $rs) {
            $tpl->newBlock("collection_cell");
            $tpl->assign(array(
                big_image => $dir_path . '/' . trim_url_image($rs['image']),
                image => $cache_image_path . resizeimage(192, 130, $dir_path . '/' . trim_url_image($rs['image'])),
    ...
    function getData($update = 0, $id = 0) {
        ...
        if (compile_post('autourl') == 1) {
            $url = $clsUrl->builUrl(strip_tags(compile_post('name')), ".html", compile_post('autourl'));
        } else {
