...
$tpl = new TemplatePower("skin/partner.tpl");
$tpl->prepare();
$id = intval($_GET['id']);
$idc = intval($_GET['idc']);
$pid = intval($_GET['pid']);
...
class Partner extends cat_tree {

    private $page = "Trang đối tác - khách hàng";
    private $item = "đối tác";
    private $table = "partner";
    private $id_item = "id_partner";
    private $par_page = "partner";
    private $numberPageShow = 8;
    private $numberItemPage = 30;
    private $thumbwidth = 80;
    private $data_type = 'partner';
    ...
    private function showUpdate() {
        ...
        if ($rs = $DBi->fetch_array($db)) {
            ...
            if ($rs['icon']) {
                $tpl->assign("imageurladv", $rs['icon']);
                $tpl->assign("imageadv", '<img src="image.php?w=40&src=' . $dir_image . "/" . $rs['icon'] . '" class="img-polaroid marginright5" align="left"  />');
            }
            if ($rs['active'] == 1)
                $tpl->assign("active", "checked");
            if ($rs['btnregist'] == 1)
                $tpl->assign("btnregist", "checked");
        }
    }
    ...
    function getData($update = 0, $id = 0) {
        global $my, $_FILES, $url_extension_article, $clsUrl, $CONFIG;
        $id = intval($id);
        $data = array();
        ...
        $data['company'] = compile_post('company');
        ...
        $data['last_modify'] = time() + $CONFIG['time_offset'];
