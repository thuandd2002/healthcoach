...
$tpl = new TemplatePower("skin/daily.tpl");
$tpl->prepare();
$id = intval($_GET['id']);
$idc = intval($_GET['idc']);
$pid = intval($_GET['pid']);
...
class Daily extends cat_tree {

    private $page = "Hệ thống đai lý";
    private $item = "đại lý";
    private $table = "daily";
    private $id_item = "id_daily";
    private $par_page = "daily";
    private $numberPageShow = 8;
    private $numberItemPage = 30;
    private $thumbwidth = 80;
    private $data_type = 'daily';
    ...
    private function showAddNew() {
        ...
        $tpl->assign("provinceList", $this->getProvinceList());
        $tpl->assign("districtid", "0");
    ...
    private function showUpdate() {
        ...
        if ($rs = $DBi->fetch_array($db)) {
            ...
            $tpl->assign("provinceList", $this->getProvinceList($rs['provinceid']));
            $tpl->assign("districtid", intval($rs['districtid']));
            $tpl->assign("phone", $rs['phone']);
            $tpl->assign("email", $rs['email']);
            $tpl->assign("website", $rs['website']);
            ...
            if ($rs['image']) {
                $tpl->assign("image", '<img width="160" src="' . $rs['image'] . '" />');
            }
            $tpl->assign("imageurl", $rs['image']);
            if ($rs['active'] == 1)
                $tpl->assign("active", "checked");
            if ($rs['phanphoi'] == 1)
                $tpl->assign("phanphoi", "checked");
            if ($rs['baohanh'] == 1)
                $tpl->assign("baohanh", "checked");
        }
    }
    ...
    private function showList() {
        ...
        while ($rs = $DBi->fetch_array($db['db'])) {
            ...
            if ($rs['image']) {
                $tpl->assign("image", '<img width="80" src="' . $rs['image'] . '" class="thumblistnews" />');
            }
            if ($rs['active'] == 1)
                $tpl->assign("active", "checked");
    ...
    private function save() {
        global $DBi, $DBi, $tpl, $lang, $dklang, $clsUrl, $search_obj;
        $data = $this->getData();
        if ($data) {
    ...
    public function getProvinceList($provinceid = 0) {
        global $DBi, $tpl, $lang;
        $sql = "SELECT * FROM vn_province ORDER BY thu_tu,provinceid, name ASC";
        $db = $DBi->query($sql);
        $str = '<option value="">---- Chọn tỉnh/TP ----</option>';
        while ($rs = $DBi->fetch_array($db)) {
            if ($provinceid == $rs['provinceid']) {
                $str .= '<option selected value="' . $rs['provinceid'] . '"> ' . $rs['name'] . '</option>';
            } else {
                $str .= '<option  value="' . $rs['provinceid'] . '">' . $rs['name'] . '</option>';
            }
        }
        return $str;
    }
    ...
    function getData($update = 0, $id = 0) {
        ...
        $data['provinceid'] = intval(compile_post('provinceid'));
        $data['districtid'] = intval(compile_post('districtid'));
        $data['maptitle'] = compile_post('maptitle');
        ...
        $data['website'] = compile_post('website');
        $data['linkmap'] = compile_post('linkmap');
        $data['address'] = compile_post('address');
        $data['phanphoi'] = intval(compile_post('phanphoi'));
        $data['baohanh'] = intval(compile_post('baohanh'));
