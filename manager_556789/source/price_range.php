...
$tpl = new TemplatePower("skin/price_range.tpl");
$tpl->prepare();
$id = intval($_GET['id']);
$idc = intval($_GET['idc']);
$pid = intval($_GET['pid']);
$module = new price_range();
$module->_int();
$tpl->printToScreen();

class price_range extends cat_tree {
    private $page = "Khoảng giá";
    private $item = "khoảng giá";
    private $table = "price_range";
    private $id_item = "id";
    private $par_page = "price_range";
    private $numberPageShow = 8;
    private $numberItemPage = 30;
    private $thumbwidth = 80;
    private $data_type = 'price_range';
    ...
    private function showUpdate() {
        ...
        if ($rs = $DBi->fetch_array($db)) {
            ...
            $tpl->assign("min_value", $rs['min_value']);
            $tpl->assign("max_value", $rs['max_value']);
    ...
    function getData($update = 0, $id = 0) {
        ...
        $data['min_value'] = compile_post('min_value');
        $data['max_value'] = compile_post('max_value');
        $data['lang'] = $lang;
        //$data['ngay_dang']	= time();
