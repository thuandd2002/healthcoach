...
$tpl = new TemplatePower("skin/info_donvi.tpl");
$tpl->prepare();
$id = intval($_GET['id']);
$idc = intval($_GET['idc']);
$pid = intval($_GET['pid']);
...
class Info extends cat_tree {
    ...
    private $page = "Trang Cơ sở vật chất";
    private $item = "nội dung";
    private $table = "info_donvi";
    private $id_item = "id_info";
    private $par_page = "info_donvi";
    private $numberPageShow = 8;
    private $numberItemPage = 30;
    private $data_type = 'info_donvi';
    ...
    private function showUpdate() {
        ...
        if ($rs = $DBi->fetch_array($db)) {
            ...
            if ($rs['image']) {
                $tpl->assign("image", '<img src="' . $rs['image'] . '" class="img-polaroid marginright5" align="left" />');
            }
